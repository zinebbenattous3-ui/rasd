...
export const Route = createFileRoute("/inspector/reports")({
  head: () => ({
    meta: [
      { title: "Rapports d'Inspection Régionale — Rased" },
      {
        name: "description",
        content: "Génération sécurisée de rapports sanitaires d'inspection wilayale au format PDF et Excel.",
      },
    ],
  }),
...
export function InspectorReportsPage() {
  ...
  // Validate Authentication & Role Enforcement on Mount (STRICT: INSPECTOR ONLY)
  useEffect(() => {
    async function verifyAuth() {
      const authResult = await validateCurrentSession(["INSPECTOR"]);
      if (!authResult.authorized) {
        // Redirect to login if unauthenticated or unauthorized role
      ...
      if (authResult.user) {
        ...
        // Fetch inspector record to determine assigned Wilaya
        ...
        if (inspRec?.wilaya) {
          ...
          setErrorMsg("Profil d'inspecteur incomplet : Wilaya non attribuée.");
        }
      }
  ...
  // Fetch Diseases and Facilities for Dropdowns
  useEffect(() => {
  ...
  const fetchReport = async () => {
    ...
    try {
      // Note: Server-side enforces role === INSPECTOR and forces forcedWilaya = inspector.wilaya
  ...
  // Initial Data Load once auth is confirmed
  useEffect(() => {
    if (!authChecking && currentUser && currentUser.role === "INSPECTOR") {
      fetchReport();
    }
  ...
  const handleGeneratePDF = async () => {
    ...
    setTimeout(() => {
      try {
        ...
        setExportSuccessMsg("✓ Document PDF d'inspection généré et téléchargé");
      } catch (err: any) {
  ...
  const handleGenerateExcel = async () => {
    ...
    setTimeout(() => {
      try {
        ...
        setExportSuccessMsg("✓ Classeur Excel d'inspection exporté avec succès");
      } catch (err: any) {
    ...
    return (
      <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", color: COLORS.navy }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: "1.05rem", fontWeight: "700" }}>Vérification des habilitations d'inspecteur...</div>
        </div>
      </div>
    );
  }

  // Filter facilities limited to Inspector's Wilaya
  const filteredFacilities = facilitiesList.filter((f) => {
    if (inspectorWilaya) {
      return f.wilaya === inspectorWilaya || f.wilaya?.startsWith(inspectorWilaya);
    }
    return true;
  });

  const assignedWilayaObj = ALGERIA_WILAYAS_69.find(w => w.code === inspectorWilaya);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
        ...
        >
          <div style={{ padding: "10px", borderRadius: "12px", backgroundColor: "#FEF3C7", color: "#B45309" }}>
            <MapPin size={22} />
          </div>

          <div>
            <div style={{ fontSize: "0.72rem", color: COLORS.muted, fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Périmètre Strict de la Wilaya
            </div>
            <div style={{ fontSize: "0.95rem", fontWeight: "800", color: COLORS.navy, display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <span>Wilaya {inspectorWilaya || "—"} {assignedWilayaObj ? `(${assignedWilayaObj.name})` : ""}</span>
            </div>
            <div style={{ fontSize: "0.75rem", color: COLORS.teal, fontWeight: "600", marginTop: "2px" }}>
              Rapports strictement restreints par le serveur
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "14px" }}>
          <div style={{ backgroundColor: "white", padding: "18px", borderRadius: "16px", border: `1px solid ${COLORS.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.75rem", color: COLORS.muted, fontWeight: "700", textTransform: "uppercase" }}>Événements Wilaya</span>
          ...
          <div style={{ backgroundColor: "white", padding: "18px", borderRadius: "16px", border: `1px solid ${COLORS.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.75rem", color: COLORS.muted, fontWeight: "700", textTransform: "uppercase" }}>Établissements Actifs</span>
      ...
      <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px" }}>
        <div style={{ fontSize: "1rem", fontWeight: "800", color: COLORS.navy, marginBottom: "14px", display: "flex", alignItems: "center", gap: "8px" }}>
          <Layers size={18} color={COLORS.teal} /> Modèles de Rapports d'Inspection
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "12px" }}>
          {[
            { type: "EXECUTIVE", icon: BarChart3, title: "Synthèse Wilaya", desc: "Bilan d'inspection générale territorial" },
            { type: "FACILITY", icon: Building2, title: "Par Établissement", desc: "Structures de la wilaya" },
            { type: "DISEASE", icon: Activity, title: "Par Pathologie", desc: "Suivi des maladies signalées" },
            { type: "WILAYA", icon: MapPin, title: "Par Gravité", desc: "Analyse des urgences sanitaires" },
            { type: "DETAILED_EVENTS", icon: FileText, title: "Registre Détaillé", desc: "Fiches d'inspection et preuves" },
          ].map((card) => {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }} className="lg:grid-cols-4">
        ...
        <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${COLORS.border}`, paddingBottom: "10px" }}>
            <div style={{ fontSize: "0.95rem", fontWeight: "800", color: COLORS.navy, display: "flex", alignItems: "center", gap: "6px" }}>
              <Filter size={16} color={COLORS.teal} /> Filtres Régionaux
            </div>
          ...
          <div>
            ...
            >
              <Lock size={14} color="#B45309" />
              <span>Wilaya {inspectorWilaya || "16"} (Restreint)</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-3" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          >
            <div>
              <div style={{ fontSize: "0.75rem", color: COLORS.teal, fontWeight: "800", textTransform: "uppercase" }}>
                Rapport d'Inspection Prêt
              </div>
              <h2 style={{ fontSize: "1.35rem", fontWeight: "800", margin: 0, color: "white" }}>
                {reportData?.summary.reportTitle || "Rapport Sanitaire Wilaya"}
              </h2>
              <div style={{ fontSize: "0.82rem", color: "#94a3b8", marginTop: "2px" }}>
                {reportData ? `${reportData.summary.totalEvents} signalements dans la Wilaya ${inspectorWilaya}.` : "Chargement..."}
              </div>
            </div>
          ...
          <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px" }}>
            ...
            {/* TAB CONTENT: SUMMARY / OTHER TABS */}
            {activeTab !== "events" && reportData && (
              <div style={{ fontSize: "0.88rem", color: COLORS.text }}>
                <p>Données d'inspection pour la Wilaya <strong>{inspectorWilaya}</strong> enregistrées avec succès.</p>
