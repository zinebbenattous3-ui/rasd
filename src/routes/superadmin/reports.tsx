...
export const Route = createFileRoute("/superadmin/reports")({
  head: () => ({
    meta: [
      { title: "Observatoire National & Rapports Superadmin — Rased" },
      {
        name: "description",
        content: "Observatoire national de surveillance épidémiologique et génération de bilans consolidés (69 Wilayas).",
      },
    ],
  }),
...
export function SuperadminReportsPage() {
  ...
  // Initial Data Load
  useEffect(() => {
    if (!authChecking && currentUser && currentUser.role === "SUPERADMIN") {
      fetchReport();
    }
    ...
    return (
      <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", color: COLORS.navy }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: "1.05rem", fontWeight: "700" }}>Vérification des accès Administrateur Suprême...</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
        <div>
          <h1 style={{ fontSize: "1.8rem", fontWeight: "900", color: COLORS.navy, letterSpacing: "-0.02em", margin: 0 }}>
            🌐 Observatoire National & Bilan Superadmin
          </h1>
          <p style={{ color: COLORS.muted, fontSize: "0.92rem", marginTop: "4px" }}>
            Consolidation et surveillance sanitaire globale sur l'ensemble des 69 wilayas de la République.
          </p>
        </div>
      ...
      <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px" }}>
        <div style={{ fontSize: "1rem", fontWeight: "800", color: COLORS.navy, marginBottom: "14px", display: "flex", alignItems: "center", gap: "8px" }}>
          <Layers size={18} color={COLORS.teal} /> Modèles de Rapports Nationaux
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "12px" }}>
          {[
            { type: "EXECUTIVE", icon: BarChart3, title: "Synthèse Nationale", desc: "Bilan général de l'Observatoire" },
            { type: "FACILITY", icon: Building2, title: "Par Établissement", desc: "Structures de l'ensemble du territoire" },
            { type: "DISEASE", icon: Activity, title: "Par Pathologie", desc: "Répartition nationale des maladies" },
            { type: "WILAYA", icon: MapPin, title: "Par Wilaya / Gravité", desc: "Analyse géographique des 69 Wilayas" },
            { type: "DETAILED_EVENTS", icon: FileText, title: "Registre Détaillé", desc: "Registres complets et pièces justificatives" },
          ].map((card) => {
        ...
        <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${COLORS.border}`, paddingBottom: "10px" }}>
            <div style={{ fontSize: "0.95rem", fontWeight: "800", color: COLORS.navy, display: "flex", alignItems: "center", gap: "6px" }}>
              <Filter size={16} color={COLORS.teal} /> Filtres Nationaux
            </div>
          ...
          >
            <div>
              <div style={{ fontSize: "0.75rem", color: COLORS.teal, fontWeight: "800", textTransform: "uppercase" }}>
                Observatoire National Prêt
              </div>
              <h2 style={{ fontSize: "1.35rem", fontWeight: "800", margin: 0, color: "white" }}>
                {reportData?.summary.reportTitle || "Bilan National Sanitaire"}
              </h2>
              <div style={{ fontSize: "0.82rem", color: "#94a3b8", marginTop: "2px" }}>
                {reportData ? `${reportData.summary.totalEvents} signalements répertoriés à l'échelle nationale.` : "Chargement..."}
              </div>
            </div>
          ...
          <div style={{ backgroundColor: "white", borderRadius: "18px", border: `1px solid ${COLORS.border}`, padding: "20px" }}>
            ...
            {activeTab !== "events" && reportData && (
              <div style={{ fontSize: "0.88rem", color: COLORS.text }}>
                <p>Consolidation de l'Observatoire National Sanitaire.</p>
